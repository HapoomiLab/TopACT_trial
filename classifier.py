...
def normalize_rows(matrix: scipy.sparse.spmatrix | np.matrix,
    ...
    return sparsetools.rescale_rows(matrix, factors)
...
def train_from_countmatrix(classifier: Classifier,
                           countmatrix: countdata.CountMatrix,
                           label: str
                           ):
...
class KNNClassifier(Classifier):
    """Based on the implementation of Abdelaal et al. 2019, replaced with K-Nearest Neighbors."""

    def __init__(self, r_value: int = 5, n_neighbors: int = 5):
        super().__init__()
        scaler = StandardScaler(with_mean=False)
        clf = Pipeline([('scaler', scaler), ('clf', KNeighborsClassifier(n_neighbors=n_neighbors))])
        self.clf = CalibratedClassifierCV(clf)
        self.r_value = r_value
...
class PCASVCClassifier(Classifier):
    """Based on the implementation of Abdelaal et al. 2019"""
    ...
    def __init__(self, r_value: int = 5):
        ...
        clf = Pipeline([('scaler', scaler), ('pca',pca),('clf', LinearSVC(dual=False))])
        self.clf = CalibratedClassifierCV(clf)
        self.r_value = r_value
...
class SVD_SVCClassifier(Classifier):
    """Based on the implementation of Abdelaal et al. 2019, using TruncatedSVD for sparse input."""

    def __init__(self, r_value: int = 5, n_components: int = 5):
        super().__init__()
        scaler = StandardScaler(with_mean=False)
        svd = TruncatedSVD(n_components=n_components)
        clf = Pipeline([('scaler', scaler), ('svd', svd), ('clf', LinearSVC(dual=False))])
        self.clf = CalibratedClassifierCV(clf)
        self.r_value = r_value
